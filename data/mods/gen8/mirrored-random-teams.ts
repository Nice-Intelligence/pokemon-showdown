...
export class RandomGen8Teams {
	...
	setSeed(prng?: PRNG | PRNGSeed) {
		this.prng = this.prng; // this is to by pass random ass setSeed call. no clue what calls also dont care  //&& !Array.isArray(prng) ? prng : new PRNG(prng);
	}
	...
	randomTeam() {
		...
		const seed = this.prng.initialSeed;
		this.prng = new PRNG(this.prng.initialSeed)
		const ruleTable = this.dex.formats.getRuleTable(this.format);
